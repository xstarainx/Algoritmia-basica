...
int main(int argc, char *argv[]) {
	...
	if(tipo=="-c"){
		...
		string nom_fichDesc = nom_fich.substr(0, nom_fich.length()-(pos+1));
			...
			if(f4.is_open()){
				...
				while(!f.eof() && !termina) {
					while(numBits<8){
						...
						if(tabla_decod.find(clave)!=tabla_decod.end()){
								if(totalCEscritos==totalC-1){
									if((int(tabla_decod[clave])!=-1 )){
